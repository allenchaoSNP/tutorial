...
class DemoMssql(http.Controller):

    @http.route('/demo/allen', auth='user')  #路徑，auth判斷需登入才可看
    def list(self, **kwargs):
        obj = http.request.env['connect.mssql'] #使用的model
        objs = obj.search([])
        return http.request.render(
            'AllenAddons.demo_mssql_template',{'objs': objs}) #指定到demo_odoo_tutorial資料夾下template的ID為demo_odoo_template的頁面
